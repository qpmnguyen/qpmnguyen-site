...
The elo rating system is a method developed by [Arpad Elo](https://en.wikipedia.org/wiki/Arpad_Elo) in order to rank players by ability in a zero sum game. Elo has been used in various contexts, most famously the way to rank chess players through the UCSF and FIDE systems. Elo has been used in modern contexts as well, from Tinder to World of Warcraft. A really nice attribute about Elo is that a difference in scores can be translated to a probability. Let's assume a focal player $A$ with rank $R_A$ and opponent $B$ with rank $R_B$. The expected value $E_A$ of player $A$ in the match [can be defined as](https://en.wikipedia.org/wiki/Elo_rating_system#Implementing_Elo's_scheme):  
$$E_A = \frac{1}{1 + 10^{(R_B - R_A)/400}} = E_A = \frac{1}{1 + 10^{-(R_A - R_B)/400}}$$.  

Remember that the standard logistic function is $\frac{1}{1 + e^{-x}}$. You can see that there are tons of similarities between the two functions, with the exception being the exponential constant $e$. This is because while elo probabilities are log-odds, they are not natural log based but rather based on base 10. 

Through this logistic formulation (while not exactly logistic due to the differences in the base of the logs), the expected values are bounded between 0 and 1. 

In an elo model, after each match, the rankings are updated as:  
$$R_A* = R_A + K \cdot (outcome - E_A)$$
where $K$ is the K-factor controlling the relative gains and losses of elo points. The variable `outcome` here is a binary variable (1 if player $A$ wins). Think of this similar to how you would use a binary cross-entropy loss to train a model that outputs log odds (such as a logistic regression) and true values which are binary. The difference is that in this case, we use this to determine the new rankings based on record and expectation due to differences in elo rankings. As such, let's define a couple of functions reflecting the ideas above:
...
Let's say that there is about a 200 difference in elo, then the win probability, according to [this website](https://www.walkofmind.com/programming/chess/elo.htm) is about 76%.  
...
Before we apply this to our data, first let's process further the polling data, to assign a win or loss based upon the polling percentages. If the poll is tied, a random draw from the binomial distribution is performed. 
...
Applying our elo sequences model for all states with different $K$-factors and initializations  
...
Let's start with a reasonable assumption of $K = 50$ and the initial elo seeding of $1000$. Since elo operates mostly from the differences in elo rankings, the initial elo seeding is not as important for this application. If you are interested in building more complex elo systems with provisional placements and tons of players, then this issue would be more relevant. We're also going to set it by default to be around 1000 iterations.  
...
```
We can visualize the trajectory of elo for each state. Let's take three states: California, Pennsylvania and Arkansas. These three states represents three types of states you would usually see: a blue state, a swing state and a red state. 
